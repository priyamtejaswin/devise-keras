...
import cv2
import sqlite3
from contour_utils import file2poly
...
def query_string_to_word_indices(query_string):
	...
	for word in words:
		if word not in all_words:
			_err_msg = "Could not find the word <u><b>{}</b></u> in server's dictionary.".format(word)
...
@app.route("/index")
def index():
	return render_template("index_lime_contours.html", title="Home")
